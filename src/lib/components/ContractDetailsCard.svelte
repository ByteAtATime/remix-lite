...
<Card
	class="relative border-primary bg-background/80 {isPinned
		? 'sticky top-0 z-10 shadow-xl backdrop-blur-md'
		: ''}"
>
	<div class="bg-primary/25 p-4">
		<Button
			onclick={ondeploy}
			size="lg"
			class="float-right bg-primary text-black hover:bg-primary/90"
		...
		{#if contract && address && abi}
			<div>
				<p class="text-sm">Deployed successfully at:</p>
				<div
