<script lang="ts">
	...
	let previousAccount = $state<Address | undefined>(undefined);
	...
	$effect(() => {
		if (wallet.isConnected && wallet.address) {
			if (!previousAccount) {
				previousAccount = selectedTevmAccount;
			}
			untrack(() => updateEditorState({ selectedAccount: wallet.address as Address }));
		} else {
			if (previousAccount) {
				untrack(() => updateEditorState({ selectedAccount: previousAccount }));
